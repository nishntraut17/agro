...
const NewNavMenu = ({ isCollapsed, setIsCollapsed }: {
    ...
    return (
        <>
            {!isCollapsed &&
                <div className={`flex flex-col md:hidden h-screen w-screen fixed top-0 right-0 backdrop-blur-sm bg-[#fffefbde] px-6 py-8 transition-opacity duration-1000 ease-in-out z-[1300] ${isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-auto'}`}>
